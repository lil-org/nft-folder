async function fetchHtml(notUsedYetItem) {
    ...
    const itemsJsonString = await response.text();
    let items;
    try {
...
async function generateHtmlContent(item) {
    ...
    const tokensJsonString = await fetch(`items/tokens/${id}.json`).then(res => res.text());
    const bundledTokens = JSON.parse(tokensJsonString);

    const scriptJsonString = await fetch(`items/scripts/${id}.json`).then(res => res.text());
    const script = JSON.parse(scriptJsonString);

    const lib = (script.kind === "js" || script.kind === "svg") 
        ? "" 
        : await fetch(`items/libs/${script.kind}.js`).then(res => res.text());
