...
struct FileSaver {
    ...
    func save(name: String, metadata: MinimalTokenMetadata, tmpLocation: URL?, data: Data?, fileExtension: String, destinationURL: URL, downloadedFromURL: URL?) -> URL? {
        if fileExtension.lowercased() == "html", let downloadedFromURL = downloadedFromURL {
            ...
            return save(name: name, metadata: metadata, tmpLocation: nil, data: webloc, fileExtension: "webloc", destinationURL: destinationURL, downloadedFromURL: nil)
            ...
            if let mbJsonData = mbJsonData, let dataOrURL = extractValueFromJson(jsonData: mbJsonData) {
                ...
                case .data(let data, let fileExtension):
                    return save(name: name, metadata: metadata, tmpLocation: nil, data: data, fileExtension: fileExtension, destinationURL: destinationURL, downloadedFromURL: downloadedFromURL)
                case .url(let url):
                    return url
                }
            }
